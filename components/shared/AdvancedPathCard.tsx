...
import {
  Bot, SlidersHorizontal, Search, Layers, ShieldCheck,
  Workflow, Plug, Users, BookOpen, Clock, ArrowRight,
  CircleCheck as CheckCircle, Sparkles,
} from 'lucide-react';
import Link from 'next/link';
import type { AdvancedLearningPath } from '@/lib/data/advancedPaths';

const iconMap: Record<string, React.ElementType> = {
  Bot, SlidersHorizontal, Search, Layers, ShieldCheck, Workflow, Plug, Users, Sparkles,
};
...
const badgeColors: Record<string, string> = {
  'New in 2026':    'bg-blue-600 text-white',
  'Most Popular':   'bg-emerald-600 text-white',
  'Critical Skill': 'bg-rose-600 text-white',
  'Trending':       'bg-amber-500 text-white',
};

interface AdvancedPathCardProps {
  path: AdvancedLearningPath;
}

export function AdvancedPathCard({ path }: AdvancedPathCardProps) {
  const Icon = iconMap[path.icon] ?? Bot;
  const level = levelStyles[path.level] ?? levelStyles['Intermediate'];
